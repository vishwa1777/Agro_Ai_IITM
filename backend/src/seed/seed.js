...
const retailerSchema = new Schema(
    {
        ...
        stockLevel: {
            type: String,
            enum: ["Critical", "Low Stock", "Medium", "Healthy"],
            default: "Healthy",
        },
        alertLevel: {
            type: String,
...
const taskSchema = new Schema(
    {
        ...
        type: {
            type: String,
            enum: ["Visit", "Stock", "Revenue", "Task", "Advisory"],
            default: "Task",
        },
...
const AGRI_STORES = [
    ...
    "Surya Agro Mart",
    "Sanjivani Crop Care",
    "Hari Om Beej Bhandar",
    "Lakshmi Agri Services",
    "Durga Krishi Store",
    "Saraswati Fertilizers",
    "Ganesh Agro Hub",
    "New India Agri Depot",
    "Kishan Mitra Kendra",
    "Jeevan Agro Centre",
    "Pragati Seeds",
    "Samridhi Agri Store",
...
const VISIT_PURPOSES = [
    "Stock replenishment discussion",
    "Pending payment collection",
    "New product demo",
    "Crop advisory session",
    "Relationship maintenance",
    "Complaint resolution",
    "Seasonal order booking",
    "Field inspection",
...
async function seed() {
    ...
    await mongoose.connect(
        `${process.env.MONGODB_URI}/${process.env.MONGODB_NAME}?appName=Cluster0`
    );
    ...
    const growersData = Array.from({ length: 60 }, (_, i) => {
        ...
        const riskScore =
            riskLvl === "High" ? rand(62, 95) : riskLvl === "Medium" ? rand(35, 61) : rand(5, 34);
    ...
    console.log(
        `   ✔ ${revenues.length} revenue records seeded (24 months × ${REGIONS.length} regions)`
    );
    ...
    const buildHourly = (basePct) => {
        ...
        const pcts = Array.from({ length: 24 }, (_, i) =>
            Math.max(
                0,
                Math.min(100, basePct + rand(-20, 20) + (i > 10 && i < 18 ? rand(5, 25) : 0))
            )
        );
